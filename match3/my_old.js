...
function Falldown(matches) {
    ...
    keys.forEach(key => {
        ...
        for (let p = 0; p < matches[key].length; p++) {
            ...
            for (let i = 0; i < pair.count; i++) {
                ...
                for (let y = 0; y < copyTiles.length; y++) {
                    for (let x = 0; x < copyTiles[y].length; x++) {
                        const tile = copyTiles[y][x];

                        if (tile != null && !tile.isInPair && tile.col == pair.tiles[i].col && tile.row < pair.tiles[i].row) {
                            let plus;
                            ...
                            self.tiles[tile.row + plus][x] = tile;
                            self.tiles[tile.row][x] = pair.tiles[i];
    ...
    keys.forEach(key => {
        ...
        for (let i = matches[key].length - 1; i >= 0; i--) {
            console.log(key);
            minus = Math.min.apply(Math, matches[key][i].tiles.map(x => {
                if (x.row > 0) {
                    return x.row;
                } else {
            ...
            for (let j = 0; j < matches[key][i].count; j++) {
                let tile = matches[key][i].tiles[j];
